...
sub BUILD {
    ...
    for my $room ( @{ $self->rooms } ) {
        my $uri = sprintf "https://%s.campfirenow.com/room/$room/join", $self->account;
...
sub speak {
    ...
    my $scope = AnyEvent::HTTP::ScopedClient->new(sprintf "https://%s.campfirenow.com/room/$room/speak", $self->account);
    $scope->header({
            Authorization => $self->authorization,
            Accept => '*/*',
...
sub leave {
    my ($self, $room) = @_;
    my $scope = AnyEvent::HTTP::ScopedClient->new(sprintf "https://%s.campfirenow.com/room/$room/leave", $self->account);
    $scope->header({
            Authorization => $self->authorization,
            Accept => '*/*',
            'Content-Type' => 'application/json',
